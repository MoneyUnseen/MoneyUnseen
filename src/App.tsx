...
function App() {
  ...
  // Detect duplicates: same name+cost+frequency
  const duplicateIds = (() => {
    const seen = new Map<string, string>()
    const dupes: string[] = []
    for (const s of subscriptions) {
      const key = `${s.name.trim().toLowerCase()}|${s.cost}|${s.frequency}`
      if (seen.has(key)) {
        dupes.push(s.id) // keep first, delete subsequent
      } else {
        seen.set(key, s.id)
      }
    }
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: 'linear-gradient(160deg, #f5f3ff 0%, #faf9ff 40%, #f0f4ff 100%)' }}>
            ...
            <ExportButton
              ...
              onImport={async (imported) => {
                ...
                for (const sub of imported) {
                  const key = `${sub.name}|${sub.cost}|${sub.frequency}`
                  if (!existing.has(key)) {
                    try { await addSubscription(sub) } catch { /* skip */ }
                  }
                }
              }}
            />
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        {duplicateIds.length > 0 && !dedupDone && (
          <div style={{
            ...
            <span style={{ fontSize: '0.82rem', color: '#92400e' }}>
              ⚠️ {duplicateIds.length} duplicate item{duplicateIds.length > 1 ? 's' : ''} found — probably from an earlier import.
            </span>
            <button onClick={handleDedup} style={{
              padding: '0.35rem 0.85rem', borderRadius: 8, border: 'none',
              background: '#f59e0b', color: '#fff', fontSize: '0.8rem',
        ...
        {dedupDone && duplicateIds.length === 0 && (
          <p style={{ textAlign: 'center', fontSize: '0.8rem', color: '#16a34a' }}>✓ Duplicates removed</p>
        )}
